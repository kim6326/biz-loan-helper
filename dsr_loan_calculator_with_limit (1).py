...
st.markdown(f"▶ 입력된 시세: {apt_price:,} 원  |  LTV 비율: {ltv_ratio*100:.1f}%")
...
if loan_type == "혼합형":
    fixed_years = st.number_input(
        "↳ 고정금리 적용 기간 (년)", min_value=0, max_value=100, value=5
    )

total_years = st.number_input(
    "↳ 총 대출 기간 (년)", min_value=1, value=30
)

cycle_level = None
if loan_type == "주기형":
    st.markdown("↳ 주기형 단계 선택")
    cycle_level = st.selectbox(
        "", ["1단계", "2단계", "3단계"]
    )

new_rate = st.number_input(
    "신규 대출 금리 (%)", min_value=0.0, format="%.2f", value=4.7
)
