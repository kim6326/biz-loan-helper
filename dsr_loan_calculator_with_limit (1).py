...
def calculate_monthly_payment(principal, rate, years, repay_type="원리금균등"):
    ...
    if repay_type == "원금균등":
        ...
        return principal_part + principal * mr
    if repay_type == "만기일시":
        return principal * mr
    return 0
...
def calculate_dsr(existing_loans, annual_income):
    ...
    for loan in existing_loans:
        monthly = calculate_monthly_payment(loan['amount'], loan['rate'], loan['years'], loan['repay_type'])
...
if page == "전세대출 계산기":
    ...
    use_custom_ltv = st.checkbox("LTV 수동 입력", key="dsr_ltv_valchk")
    ...
    if loan_type == "주기형":
        ...
        st.info(f"▶ 주기형 {cycle_mon}개월 → {cycle_level}, 배율 {mult2:.1f}배")
    ...
    if st.button("계산하기", key="dsr_calc"):
        ...
        mult3 = get_stress_multiplier(loan_type, fixed_years, total_years, cycle_level)
        stress_rate3 = new_rate * mult3
        discount3 = 1.5 if region in ["서울", "경기", "인천"] else 0.75
        adj_rate3 = stress_rate3 - discount3

        new_mon3 = calculate_monthly_payment(new_amount, adj_rate3, total_years, "만기일시")
        cap3 = min(price * ltv_ratio, 600_000_000 if first_home else price * ltv_ratio)

        st.write(f"기존 월 상환액: {exist_mon2:,.0f}원")
        st.write(f"DSR 한도: {dsr_lim:,.0f}원")
        st.write(f"여유 상환액: {avail:,.0f}원")
        st.write(f"스트레스 금리 (전): {stress_rate3:.2f}% → (감면 후) {adj_rate3:.2f}% (-{discount3:.2f}%p)")
        st.write(f"신규 월 상환액: {new_mon3:
