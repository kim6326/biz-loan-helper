...
if page == "전세대출 계산기":
    ...
    if st.button("계산"):
        ...
        # 승인 여부: 한도 체크 + 금융비용부담율 체크
        within_limit = (ho <= lim)
        within_burden = (burden_pct <= burden_limit)
        ok = within_limit and within_burden

        # 디버깅용 로그
        st.write(f"▶ 한도내 여부: {within_limit}, 부담율내 여부: {within_burden}")
    ...
    for i in range(cnt2):
        ...
        yr2 = st.number_input("기간(년)", 1, 40, 10, key=f"dsr_yr{i}")
