...
if page == "전세대출 계산기":
    ...
    annual_income = comma_number_input("연소득 (원)", "dsr_income", "60000000")
    ...
    if st.button("계산하기", key="dsr_calc"):
        ...
        # 최대 대출 가능액 계산
        # 만기일시 상환 가정시: 월이자 = 원금 * (adj_rate2/100/12)
        if adj_rate2 > 0:
            max_loan_dsr = avail * 12 / (adj_rate2 / 100)
        else:
            max_loan_dsr = cap2
        max_loan = min(max_loan_dsr, cap2)
